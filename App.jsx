...
const App = () => {
  ...
  const addToCart = (product, angle = "depan") => {
    setCart(prev => {
      ...
      return [...prev, { ...product, qty: 1, angle, currentImage: product.images[angle] }];
    });
                ...
                {cart.length === 0 ? <p className="text-gray-400 text-center py-10">Keranjang kosong</p> : cart.map(item => (
                  <div key={`${item.id}-${item.angle}`} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-brand text-[10px] truncate leading-tight">{item.nama} ({item.angle})</h5>
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                      ...
                      <div className="mt-6 flex flex-wrap gap-4 items-center">
                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Ganti Sudut:</span>
                        <div className="flex gap-2">
                          {["depan", "samping", "belakang"].map(angle => (
                            <button
                              key={angle}
                              onClick={() => setActiveAngle(angle)}
                              className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all border ${activeAngle === angle ? 'bg-brand text-white border-brand shadow-md scale-105' : 'bg-white text-gray-400 border-gray-200 hover:border-accent hover:text-accent'}`}
                            >
                              {angle}
                            </button>
                          ))}
