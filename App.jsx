...
const App = () => {
  ...
  return (
    <div className="min-h-screen">
      {/* Navbar */}
      <nav className="fixed top-0 w-full z-50 glass py-3 px-6 flex justify-between items-center border-b border-white/5">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsMenuOpen(true)} className="p-2 md:hidden text-white" aria-label="Menu">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          <div className="relative hidden md:block">
            <input
              ...
              className="bg-white/5 text-white placeholder-gray-500 rounded-full py-1 px-4 text-sm focus:outline-none focus:ring-2 ring-accent border border-white/10"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <button onClick={() => setIsCartOpen(true)} className="relative p-2 text-white" aria-label="Keranjang Belanja">
      ...
      <section id="produk" className="py-20 px-6 bg-[#0a0a0a]">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-6">
            ...
            <div className="relative w-full md:w-96">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 text-white rounded-xl py-3 px-4 shadow-sm focus:outline-none focus:ring-2 ring-accent"
          ...
          <div className="mb-8">
            <p className="text-gray-400 text-sm font-medium">
              {search ? `Menampilkan ${filteredProducts.length} produk untuk "${search}"` : `Menampilkan semua ${filteredProducts.length} produk`}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${category === cat ? 'bg-accent text-white shadow-lg scale-105' : 'bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10'}`}
              >
                {cat}
              </button>
            ))}
          ...
          <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 min-h-[400px]">
            <AnimatePresence mode='popLayout'>
              {filteredProducts.length === 0 ? (
                ...
                >
                  <LucideIcon name="search-slash" className="w-16 h-16 mb-4 opacity-20" />
                  <p className="text-xl font-bold">Produk tidak ditemukan</p>
                  <p className="text-sm">Coba kata kunci lain atau reset filter.</p>
                  <button onClick={() => { setSearch(""); setCategory("Semua"); }} className="mt-6 text-accent font-bold hover:underline">Reset Pencarian</button>
                </motion.div>
                ...
                return (
                  <motion.div
                    ...
                    key={product.id}
                    className="bg-[#151515] rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-accent/5 transition-all group flex flex-col border border-white/5"
                  >
                    <div className="relative aspect-square overflow-hidden cursor-pointer" onClick={() => { setSelectedProduct(product); }}>
                      ...
                      {product.terjual > 1000 && <div className="absolute top-4 left-4 bg-accent text-white text-[10px] font-bold px-3 py-1 rounded-sm uppercase shadow-lg">TERLARIS</div>}
                    </div>
                    <div className="p-5 flex-1 flex flex-col">
                      <p className="text-xs text-gray-500 mb-1 uppercase tracking-widest">{product.kategori}</p>
                      ...
                      <div className="mt-auto space-y-2">
                        ...
                        <button onClick={() => addToCart(product)} className="w-full bg-transparent border border-white/10 hover:bg-white/5 text-white py-2 rounded-full font-bold text-sm transition-all shadow-sm">Tambah Keranjang</button>
                        <button onClick={() => { setSelectedProduct(product); }} className="w-full bg-accent hover:bg-yellow-600 text-white py-2 rounded-full font-bold text-sm transition-all shadow-sm">Beli Sekarang</button>
                      </div>
                    </div>
                  </motion.div>
                );
      ...
      <footer className="bg-[#050505] text-white py-16 px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-12">
          <div>
            ...
            <p className="text-gray-500 mb-6 leading-relaxed">Topiku Topimu. Platform e-commerce topi nomor satu dengan kualitas tanpa kompromi untuk gaya hidup urban.</p>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <>
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} className="fixed left-0 top-0 h-full w-full max-w-xs bg-[#0a0a0a] z-[90] shadow-2xl p-8 flex flex-col md:hidden border-r border-white/5">
              <div className="flex justify-between items-center mb-10">
                <h1 className="text-2xl font-black tracking-tighter text-white">TKTM</h1>
                <button onClick={() => setIsMenuOpen(false)} className="text-white"><LucideIcon name="x" className="w-6 h-6" /></button>
              </div>

              <nav className="flex flex-col gap-6 mb-auto">
                <a href="#" onClick={() => setIsMenuOpen(false)} className="text-xl font-bold text-white hover:text-accent transition-colors">Beranda</a>
                <a href="#produk" onClick={() => setIsMenuOpen(false)} className="text-xl font-bold text-white hover:text-accent transition-colors">Koleksi</a>
              </nav>

              <div className="pt-10 border-t border-white/5">
                <h5 className="font-bold mb-4 text-sm uppercase tracking-widest text-gray-500">Hubungi Kami</h5>
      ...
      <AnimatePresence>
        {isCartOpen && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed right-0 top-0 h-full w-full max-w-md bg-[#0a0a0a] z-[70] shadow-2xl p-8 flex flex-col border-l border-white/5">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-bold text-white">Keranjang</h3>
                <button onClick={() => setIsCartOpen(false)} className="text-white"><LucideIcon name="x" className="w-6 h-6" /></button>
              </div>
              <div className="flex-1 overflow-y-auto space-y-2">
                {cart.length === 0 ? <p className="text-gray-400 text-center py-10">Keranjang kosong</p> : cart.map((item, index) => (
                  <div key={`${item.id}-${item.angle}-${index}`} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    <img src={item.images[item.angle || 'depan']} className="w-10 h-10 object-cover rounded-md shadow-sm" />
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-brand text-[10px] truncate leading-tight">{item.nama} <span className="text-[8px] font-normal text-gray-400 uppercase">({item.angle})</span></h5>
              ...
              {cart.length > 0 && (
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                      ...
                      <div className="flex gap-1">
                        <input
                          ...
                          placeholder="Kode Promo"
                            className="flex-1 bg-white/5 border border-white/5 rounded-md py-1 px-2 text-[10px] focus:outline-none ring-accent text-accent font-bold uppercase min-w-0"
                          value={promoInput}
                          onChange={(e) => setPromoInput(e.target.value)}
                        />
                          <button onClick={handleApplyPromo} className="bg-accent text-white px-2 rounded-md font-bold text-[9px] whitespace-nowrap">Pasang</button>
                      </div>
                    </div>
                    <div>
                      <input
                        type="text"
                        placeholder="Nama"
                          className="w-full bg-white/5 border border-white/5 text-white rounded-md py-1 px-2 text-[10px] focus:outline-none ring-accent"
                        value={userName}
                    ...
                    <div className="flex justify-between text-[10px]">
                        <span className="text-gray-500">Total ({cart.reduce((a,b)=>a+b.qty,0)} item)</span>
      ...
      <AnimatePresence>
        {checkoutItems && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed inset-x-4 bottom-4 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 w-full md:max-w-md bg-[#111111] z-[160] rounded-[2.5rem] p-8 shadow-2xl border border-white/5">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-black text-white">Konfirmasi Pesanan</h3>
                <button onClick={() => setCheckoutItems(null)} className="bg-white/5 p-2 rounded-full text-white"><LucideIcon name="x" className="w-5 h-5" /></button>
              </div>

              <div className="space-y-6">
                <div className="bg-white/5 rounded-2xl p-4 space-y-2 border border-white/5">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Metode Pembayaran</p>
                  ...
                  <p className="text-xs text-gray-600 italic">*Konfirmasi via WhatsApp</p>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">Nama Lengkap</label>
                    <input
                      type="text" placeholder="Masukkan nama..."
                      className="w-full bg-white/5 border border-white/5 text-white rounded-xl py-3 px-4 focus:outline-none focus:ring-2 ring-accent"
                  ...
                  <div>
                    <label className="text-xs font-bold text-accent uppercase tracking-widest mb-2 block">Kode Promo (Opsional)</label>
                    <div className="flex gap-1.5">
                      <input
                        type="text" placeholder="Kode promo..."
                        className="flex-1 bg-white/5 border border-white/5 rounded-xl py-2 px-3 text-[11px] focus:outline-none focus:ring-2 ring-accent text-accent font-bold uppercase min-w-0"
                        value={promoInput} onChange={(e) => setPromoInput(e.target.value)}
                      />
                      <button onClick={handleApplyPromo} className="bg-accent text-white px-3 rounded-xl font-bold text-[10px] whitespace-nowrap">Pasang</button>
                    </div>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {selectedProduct && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 lg:inset-20 bg-[#0a0a0a] z-[110] lg:rounded-3xl overflow-hidden flex flex-col lg:flex-row shadow-2xl border border-white/5">
              {/* Product Info Section (Left) */}
              <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 bg-[#0a0a0a]">
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                      <div className="relative aspect-video md:aspect-[21/9] overflow-hidden rounded-3xl shadow-2xl border border-white/5 bg-[#151515]">
                      ...
                      <div className="mt-6 flex justify-center gap-4">
                        {['depan', 'samping', 'belakang'].map((angle) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${activeAngle === angle ? 'bg-accent text-white shadow-lg scale-105' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                          >
                            {angle === 'depan' ? 'Depan' : angle === 'samping' ? 'Samping' : 'Belakang'}
                          </button>
                        ))}
                      </div>
                    </div>
                    <div className="flex-1 min-w-0 text-white">
                      <div className="mb-8">
                        ...
                        <p className="text-gray-400 leading-relaxed text-lg md:text-xl max-w-2xl">{selectedProduct.deskripsi}</p>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[10px] font-bold text-gray-500 uppercase mb-1 tracking-widest">Material</p>
                          <p className="text-sm font-bold text-white">{selectedProduct.info.bahan}</p>
                        </div>
                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[10px] font-bold text-gray-500 uppercase mb-1 tracking-widest">Size</p>
                        ...
                        <div className="col-span-2 bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[10px] font-bold text-gray-500 uppercase mb-1 tracking-widest">Highlight</p>
              ...
              <div className="w-full lg:w-[320px] xl:w-[360px] bg-[#0a0a0a] p-6 md:p-8 lg:p-10 shrink-0 flex flex-col justify-center border-t border-white/5 lg:border-t-0">
                <div className="bg-[#151515] border border-white/5 rounded-2xl p-6 shadow-2xl sticky top-0">
                  <p className="text-2xl font-bold text-white mb-2">Rp {selectedProduct.harga.toLocaleString('id-ID')}</p>
                  <p className="text-sm text-accent font-bold mb-6 flex items-center gap-1"><LucideIcon name="check-circle" className="w-4 h-4" /> Stok Tersedia</p>

                  <div className="space-y-4">
                    <div>
                      <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1 block">Jumlah</label>
                      <div className="flex items-center justify-between bg-white/5 border border-white/5 rounded-xl p-1">
                        <button
                          ...
                          className="p-2 bg-white/10 text-white rounded-lg shadow-sm hover:text-red-500"
                        >
                        ...
                        <button
                          ...
                          className="p-2 bg-white/10 text-white rounded-lg shadow-sm hover:text-accent"
                        >
                    ...
                    <div className="pt-4 border-t border-white/5 space-y-3">
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/5 text-white rounded-xl py-2 px-4 text-sm focus:outline-none ring-accent"
                        value={userName} onChange={(e) => setUserName(e.target.value)}
                      />
                      <input
                      ...
                      <div className="flex gap-1.5">
                        <input
                          ...
                          className="flex-1 bg-white/5 border border-white/5 text-white rounded-xl py-2 px-3 text-[11px] focus:outline-none text-accent font-bold uppercase min-w-0"
                          value={promoInput} onChange={(e) => setPromoInput(e.target.value)}
                        />
                        <button onClick={handleApplyPromo} className="bg-accent text-white px-3 rounded-xl font-bold text-[10px] whitespace-nowrap">Pasang</button>
                      </div>
                    ...
                    <div className="pt-4 space-y-2">
                      ...
                      <button
                        onClick={() => {
                          const msg = generateWAMessage([{ ...selectedProduct, qty: modalQty, angle: activeAngle }]);
                          if (msg) {
                            window.open(`https://wa.me/${WA_NUMBER.replace('+', '')}?text=${msg}`, '_blank');
                            setSelectedProduct(null);
                          }
        ...
        <AnimatePresence>
          {notifications.map(n => (
            <motion.div
              ...
              className="bg-[#151515] text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 pointer-events-auto border border-white/10"
            >
