...
const PRODUCTS = [
  {
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1588850561407-ed78c282e89b?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1588850561407-ed78c282e89b?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 2,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1575425186775-b8de9a427e67?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1575425186775-b8de9a427e67?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 3,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1521369909029-2afed882baee?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1521369909029-2afed882baee?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 4,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1596455607563-ad6193f76b17?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1596455607563-ad6193f76b17?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 5,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1618354691792-d1d42acfd860?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1618354691792-d1d42acfd860?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 6,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1514327605112-b887c0e61c0a?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1514327605112-b887c0e61c0a?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 7,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1572307480813-ceb0e59d8325?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1572307480813-ceb0e59d8325?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  },
  {
    id: 8,
    ...
    images: {
      ...
      samping: "https://images.unsplash.com/photo-1556306535-0f09a537f0a3?q=80&w=1000&auto=format&fit=crop&sat=-100",
      belakang: "https://images.unsplash.com/photo-1556306535-0f09a537f0a3?q=80&w=1000&auto=format&fit=crop&hue=200"
    }
  }
];
...
const App = () => {
  ...
  const addToCart = (product) => {
    const currentImage = product.images[activeAngle];
    setCart(prev => {
      const existing = prev.find(item => item.id === product.id && item.angle === activeAngle);
      if (existing) {
        return prev.map(item => (item.id === product.id && item.angle === activeAngle) ? { ...item, qty: item.qty + 1 } : item);
      }
      return [...prev, { ...product, qty: 1, angle: activeAngle, currentImage }];
    });
  ...
  const buyNowWA = (product) => {
    const inCart = cart.find(item => item.id === product.id && item.angle === activeAngle);
    const qty = inCart ? inCart.qty : 1;
    const msg = generateWAMessage([{ ...product, qty, angle: activeAngle }]);
    if (msg) {
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} className="fixed left-0 top-0 h-full w-full max-w-xs bg-white z-[90] shadow-2xl p-8 flex flex-col md:hidden">
              ...
              <div className="mb-8 relative">
                <input
                  type="text"
                  placeholder="Cari topi..."
                  className="w-full bg-gray-100 rounded-xl py-3 px-4 text-sm focus:outline-none focus:ring-2 ring-accent"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                />
              </div>
                ...
                {cart.length === 0 ? <p className="text-gray-400 text-center py-10">Keranjang kosong</p> : cart.map(item => (
                  <div key={`${item.id}-${item.angle}`} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-brand text-[10px] truncate leading-tight">{item.nama} <span className="opacity-50 font-normal">({item.angle})</span></h5>
                ...
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                      ...
                      {/* Ganti Sudut Gallery */}
                      <div className="mt-6">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Ganti Sudut</p>
                        <div className="flex gap-4">
                          {Object.entries(selectedProduct.images).map(([angle, src]) => (
                            <button
                              key={angle}
                              onClick={() => setActiveAngle(angle)}
                              className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeAngle === angle ? 'border-accent scale-110 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'}`}
                            >
                              <img src={src} className="w-full h-full object-cover" />
                              <div className="absolute inset-0 bg-black/20 flex items-end justify-center pb-1">
                                <span className="text-[8px] text-white font-bold uppercase">{angle}</span>
                              </div>
                            </button>
                          ))}
                        </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center justify-between bg-gray-50 border border-gray-100 rounded-xl p-1">
                        <button
                          onClick={() => {
                            const inCart = cart.find(i => i.id === selectedProduct.id && i.angle === activeAngle);
                            if (inCart) updateQuantity(selectedProduct.id, activeAngle, -1);
                          }}
                          className="p-2 bg-white rounded-lg shadow-sm hover:text-red-500 disabled:opacity-30"
                          disabled={!cart.find(i => i.id === selectedProduct.id && i.angle === activeAngle)}
                        >
                          <LucideIcon name="minus" className="w-4 h-4" />
                        </button>
                        <span className="font-bold text-brand">{cart.find(i => i.id === selectedProduct.id && i.angle === activeAngle)?.qty || 0}</span>
                        <button
                          onClick={() => {
                            const inCart = cart.find(i => i.id === selectedProduct.id && i.angle === activeAngle);
                            if (inCart) updateQuantity(selectedProduct.id, activeAngle, 1);
