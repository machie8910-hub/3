                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto space-y-2">
                        ...
                        <button onClick={() => addToCart(product, 'depan')} className="w-full bg-white border border-gray-200 hover:bg-gray-50 text-brand py-2 rounded-full font-bold text-sm transition-all shadow-sm">Tambah Keranjang</button>
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} className="fixed left-0 top-0 h-full w-full max-w-xs bg-white z-[90] shadow-2xl p-8 flex flex-col md:hidden">
              ...
              <nav className="flex flex-col gap-6 mb-auto">
                ...
                <div className="relative w-full mt-4">
                  <input
                    type="text" placeholder="Cari topi..."
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl py-3 px-4 focus:outline-none focus:ring-2 ring-accent text-brand"
                    value={search} onChange={(e) => setSearch(e.target.value)}
                  />
                  <LucideIcon name="search" className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                </div>
              </nav>
                ...
                {cart.length === 0 ? <p className="text-gray-400 text-center py-10">Keranjang kosong</p> : cart.map(item => (
                  <div key={`${item.id}-${item.angle}`} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-brand text-[10px] truncate leading-tight">{item.nama} <span className="text-[8px] opacity-50 uppercase">({item.angle})</span></h5>
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                          {['depan', 'samping', 'belakang'].map((angle) => (
                            <button
                              ...
                              className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${activeAngle === angle ? 'bg-black text-white shadow-lg' : 'bg-white/80 text-black hover:bg-white'}`}
                            >
                              {angle}
                            </button>
                          ))}
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm sticky top-0">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 space-y-2">
                      <button
                        onClick={() => {
                          for (let i = 0; i < modalQty; i++) {
                            addToCart(selectedProduct, activeAngle);
                          }
                        }}
                      ...
                      <button
                        onClick={() => buyNowWA({ ...selectedProduct, qty: modalQty, angle: activeAngle })}
