    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[110vh] flex items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 text-center px-4" style={{ transform: `translateY(${scrollPos * 0.2}px)` }}>
          <motion.h2 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-white text-6xl md:text-9xl font-black mb-6 tracking-tighter">TOPIKU TOPIMU</motion.h2>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-gray-300 text-xl md:text-3xl max-w-3xl mx-auto font-medium leading-relaxed">Koleksi eksklusif untuk melengkapi gaya harianmu. TKTM hadir untuk kenyamanan dan estetika premium.</motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="mt-12">
            <a href="#produk" className="bg-accent hover:bg-yellow-600 text-white font-black py-4 px-12 rounded-full transition-all shadow-2xl hover:scale-105 active:scale-95 inline-block text-lg">Jelajahi Koleksi</a>
          </motion.div>
        </div>
      </section>
                      ...
                      <div className="flex gap-4 justify-center md:justify-start">
                        {['depan', 'samping', 'belakang'].map((angle) => (
                          <button
                            ...
                            className={`relative w-20 aspect-square rounded-xl overflow-hidden border-2 transition-all ${activeAngle === angle ? 'border-accent ring-2 ring-accent/20 scale-105 shadow-lg' : 'border-gray-100 hover:border-gray-300 opacity-60 hover:opacity-100'}`}
                          >
                            <img src={selectedProduct.images[angle]} className="w-full h-full object-cover" />
                            <span className="absolute bottom-0 inset-x-0 bg-black/40 text-white text-[8px] font-bold uppercase py-1 text-center backdrop-blur-sm">
