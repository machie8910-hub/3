  ...
  return (
    <div className="min-h-screen">
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto space-y-2">
                        ...
                        <button onClick={() => { setSelectedProduct(product); }} className="w-full bg-black hover:bg-zinc-800 text-white py-2 rounded-full font-bold text-sm transition-all shadow-sm">Beli Sekarang</button>
                      </div>
                    </div>
                  </motion.div>
                );
                  ...
                  <div key={`${item.id}-${item.angle}`} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-1">
                        ...
                        <span className="text-[8px] bg-gray-100 px-1 rounded text-gray-500 uppercase">{item.angle || 'depan'}</span>
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div className="pt-6 border-t border-dashed">
                  ...
                  <button onClick={handleFinalCheckout} className="w-full bg-black text-white py-4 rounded-2xl font-bold shadow-lg hover:bg-zinc-800 transition-all flex items-center justify-center gap-2">
                    <LucideIcon name="phone" className="w-5 h-5" /> Pasang
                  </button>
                </div>
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {selectedProduct && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => { setSelectedProduct(null); setActiveAngle('depan'); setModalQty(1); }} className="fixed inset-0 bg-black/80 z-[100] backdrop-blur-md" />
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed inset-0 lg:inset-20 bg-white z-[110] lg:rounded-3xl overflow-hidden flex flex-col lg:flex-row shadow-2xl border border-gray-100">
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-12 bg-white">
                <div className="max-w-4xl mx-auto">
                  <button onClick={() => { setSelectedProduct(null); setActiveAngle('depan'); setModalQty(1); }} className="mb-6 inline-flex items-center gap-1.5 text-[11px] font-bold text-accent hover:opacity-80 transition-opacity uppercase tracking-widest">
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                      ...
                      {/* Angle Switching Gallery */}
                      <div className="flex gap-4 mt-6">
                        {['depan', 'samping', 'belakang'].map((angle) => (
                          <button
                            key={angle}
                            onClick={() => setActiveAngle(angle)}
                            className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeAngle === angle ? 'border-accent ring-2 ring-accent/20' : 'border-transparent hover:border-gray-200'}`}
                          >
                            <img src={selectedProduct.images[angle]} className="w-full h-full object-cover" />
                            <div className={`absolute inset-0 flex items-center justify-center bg-black/40 text-[10px] text-white font-bold uppercase tracking-tighter opacity-0 hover:opacity-100 transition-opacity ${activeAngle === angle ? 'opacity-100' : ''}`}>
                              {angle}
                            </div>
                          </button>
                        ))}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 space-y-2">
                      <button
                        ...
                        className="w-full bg-black hover:bg-zinc-800 text-white py-3 rounded-full font-bold text-sm shadow-sm transition-all"
                      >
                        Tambah ke Keranjang
                      </button>
                      <button
                        onClick={() => buyNowWA(selectedProduct, activeAngle, modalQty)}
                        className="w-full bg-green-500 hover:bg-green-600 text-white py-3 rounded-full font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2"
                      >
                        Beli Sekarang (COD)
                      </button>
                    </div>
