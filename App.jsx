...
const App = () => {
  ...
  const handleApplyPromo = () => {
    ...
    if (promos[code]) {
      ...
      showNotification(`GAGAL: Kode promo "${code.toUpperCase()}" tidak valid atau sudah kedaluwarsa.`);
    }
  };
                      ...
                      <div className="flex gap-4 mt-6 justify-center">
                        {['depan', 'samping', 'belakang'].map((angle) => (
                          <button
                            ...
                            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeAngle === angle ? 'bg-brand text-white shadow-lg scale-105' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
                          >
                            {angle === 'depan' ? 'Sudut Depan' : angle === 'samping' ? 'Sudut Samping' : 'Sudut Belakang'}
                          </button>
                        ))}
                      </div>
                    </div>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm sticky top-0">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 space-y-2">
                      ...
                      <button
                        onClick={() => buyNowWA({ ...selectedProduct, angle: activeAngle, qty: modalQty })}
