                ...
                {cart.length === 0 ? <p className="text-gray-400 text-center py-10">Keranjang kosong</p> : cart.map(item => (
                  <div key={item.id} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-brand text-xs truncate leading-tight">{item.nama}</h5>
                      <p className="text-[10px] text-accent font-bold mb-0.5">Rp {item.harga.toLocaleString('id-ID')}</p>
                      ...
                      <div className="flex justify-center gap-4">
                        {Object.keys(selectedProduct.images).map((angle) => (
                          <button
                            ...
                            className={`group relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeAngle === angle ? 'border-accent scale-105 shadow-lg' : 'border-transparent hover:border-gray-300'}`}
                          >
                            <img src={selectedProduct.images[angle]} className="w-full h-full object-cover" />
                            <div className={`absolute inset-0 bg-black/20 flex items-center justify-center transition-opacity ${activeAngle === angle ? 'opacity-0' : 'opacity-100 group-hover:opacity-0'}`}>
                              <span className="text-[10px] text-white font-bold uppercase">{angle}</span>
                            </div>
                          </button>
                        ))}
                      </div>
