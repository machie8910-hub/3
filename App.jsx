...
const App = () => {
  ...
  const addToCart = (product, angle = 'depan', q = 1) => {
    const currentImage = product.images[angle];
    setCart(prev => {
      const existing = prev.find(item => item.id === product.id && item.angle === angle);
      if (existing) {
        return prev.map(item => (item.id === product.id && item.angle === angle) ? { ...item, qty: item.qty + q } : item);
      }
      return [...prev, { ...product, angle, currentImage, qty: q }];
    });
  ...
  const buyNowWA = (product, angle = 'depan', q = 1) => {
    const msg = generateWAMessage([{ ...product, qty: q, angle }]);
    if (msg) {
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3 z-10">
                          {Object.entries(selectedProduct.images).map(([angle, src]) => (
                            <button
                              ...
                              className={`w-14 h-14 rounded-xl overflow-hidden border-2 transition-all ${activeAngle === angle ? 'border-accent scale-110 shadow-lg' : 'border-white/50 opacity-70 hover:opacity-100'}`}
                            >
