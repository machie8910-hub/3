...
const App = () => {
  ...
  useEffect(() => { if (selectedProduct) setActiveAngle("depan"); }, [selectedProduct]);
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 glass p-2 rounded-2xl">
                          {['depan', 'samping', 'belakang'].map(angle => (
                            <button
                              ...
                              className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${activeAngle === angle ? 'bg-brand text-white shadow-lg' : 'hover:bg-white/50'}`}
                            >
                              {angle}
                            </button>
                          ))}
