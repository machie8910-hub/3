...
const App = () => {
  ...
  const addToCart = (product, angle = "depan", quantity = 1) => {
    setCart(prev => {
      const existing = prev.find(item => item.id === product.id && item.angle === angle);
      if (existing) {
        return prev.map(item => (item.id === product.id && item.angle === angle) ? { ...item, qty: item.qty + quantity } : item);
      }
      return [...prev, { ...product, angle, qty: quantity }];
    });
  ...
  const buyNowWA = (product) => {
    const msg = generateWAMessage([{ ...product, angle: "depan", qty: 1 }]);
    if (msg) {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative group max-w-4xl mx-auto">
            <div className="overflow-hidden rounded-[2.5rem] shadow-2xl">
              ...
              >
                {recs.map((product) => (
                  <div key={product.id} className="min-w-full relative aspect-[16/9] md:aspect-[2/1] overflow-hidden cursor-pointer" onClick={() => { setSelectedProduct(product); setActiveAngle("depan"); setModalQty(1); }}>
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto space-y-2">
                        ...
                        <button onClick={() => addToCart(product, "depan", 1)} className="w-full bg-white border border-gray-200 hover:bg-gray-50 text-brand py-2 rounded-full font-bold text-sm transition-all shadow-sm">Tambah Keranjang</button>
                        <button onClick={() => { setSelectedProduct(product); setActiveAngle("depan"); setModalQty(1); }} className="w-full bg-yellow-400 hover:bg-yellow-500 text-brand py-2 rounded-full font-bold text-sm transition-all shadow-sm">Beli Sekarang</button>
                      </div>
                    </div>
                  </motion.div>
                );
                ...
                {cart.length === 0 ? <p className="text-gray-400 text-center py-10">Keranjang kosong</p> : cart.map(item => (
                  <div key={`${item.id}-${item.angle}`} className="flex gap-2 items-center border-b border-gray-50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <h5 className="font-bold text-brand text-[10px] truncate leading-tight">{item.nama} <span className="text-[8px] font-normal text-gray-400 capitalize">({item.angle})</span></h5>
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                          {['depan', 'samping', 'belakang'].map(angle => (
                            <button
                              ...
                              className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all shadow-lg ${activeAngle === angle ? 'bg-brand text-white scale-110' : 'bg-white/80 text-brand hover:bg-white'}`}
                            >
                      ...
                      <div className="mt-4 flex items-center justify-center gap-4">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Ganti Sudut:</p>
                        <div className="flex gap-2">
                          {Object.entries(selectedProduct.images).map(([angle, url]) => (
                            <button
                              key={angle}
                              onClick={() => setActiveAngle(angle)}
                              className={`w-12 h-12 rounded-lg overflow-hidden border-2 transition-all ${activeAngle === angle ? 'border-accent scale-110' : 'border-transparent opacity-60 hover:opacity-100'}`}
                            >
                    ...
                    <div className="pt-4 space-y-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (msg) window.open(`https://wa.me/${WA_NUMBER.replace('+', '')}?text=${msg}`, '_blank');
                        }}
