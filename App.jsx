...
const Motion = window.Motion || {
  motion: {
    div: ({ style, ...props }) => {
      const s = { ...style };
      const getValue = (v) => (v && typeof v === 'object' && v.get) ? v.get() : v;
      let t = s.transform || '';
      if (s.y !== undefined) {
        const yVal = getValue(s.y);
        t += ` translateY(${typeof yVal === 'number' ? yVal + 'px' : yVal})`;
        delete s.y;
      }
      if (s.scale !== undefined) {
        const sVal = getValue(s.scale);
        t += ` scale(${sVal})`;
        delete s.scale;
      }
      return <div style={{ ...s, transform: t.trim() }} {...props} />;
    },
    h2: (props) => <h2 {...props} />,
    p: (props) => <p {...props} />,
    img: (props) => <img {...props} />,
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                      <div className="relative aspect-video md:aspect-[21/9] overflow-hidden rounded-3xl shadow-2xl border border-gray-50">
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3 bg-black/40 backdrop-blur-md p-2 rounded-2xl border border-white/20">
                          {['depan', 'samping', 'belakang'].map((angle) => (
                            <button
                              key={angle}
                              onClick={() => setActiveAngle(angle)}
                              className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all ${activeAngle === angle ? 'bg-white text-brand' : 'text-white hover:bg-white/20'}`}
                            >
                              {angle}
                            </button>
                          ))}
