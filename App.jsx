...
const App = () => {
  ...
  const addToCart = (product, qty = 1) => {
    setCart(prev => {
      ...
      return [...prev, { ...product, qty, currentImage: product.images.depan }];
    });
  ...
  const buyNowWA = (product) => {
    const msg = generateWAMessage([{ ...product, qty: modalQty }]);
    if (msg) {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative group max-w-4xl mx-auto">
            <div className="overflow-hidden rounded-[2.5rem] shadow-2xl">
              ...
              >
                {recs.map((product) => (
                  <div key={product.id} className="min-w-full relative aspect-[16/9] md:aspect-[2/1] overflow-hidden cursor-pointer" onClick={() => { setSelectedProduct(product); setModalQty(1); setActiveAngle('depan'); }}>
            ...
            <AnimatePresence mode='popLayout'>
              {filteredProducts.map(product => {
                return (
                  ...
                  >
                    <div className="relative aspect-square overflow-hidden cursor-pointer" onClick={() => { setSelectedProduct(product); setModalQty(1); setActiveAngle('depan'); }}>
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto space-y-2">
                        ...
                        <button onClick={() => { setSelectedProduct(product); setModalQty(1); setActiveAngle('depan'); }} className="w-full bg-yellow-400 hover:bg-yellow-500 text-brand py-2 rounded-full font-bold text-sm transition-all shadow-sm">Beli Sekarang</button>
                      </div>
                    </div>
                  </motion.div>
                );
                  ...
                  <div className="flex flex-col gap-8">
                    <div className="w-full">
                        ...
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                          {['depan', 'samping', 'belakang'].map(angle => (
                            <button
                              ...
                              className={`w-12 h-12 rounded-lg border-2 overflow-hidden transition-all ${activeAngle === angle ? 'border-accent scale-110 shadow-lg' : 'border-white/50 opacity-70 hover:opacity-100'}`}
                            >
                      ...
                      <div className="mt-4 flex justify-center gap-6">
                        <button onClick={() => setActiveAngle('depan')} className={`text-[10px] font-black uppercase tracking-widest ${activeAngle === 'depan' ? 'text-accent' : 'text-gray-400'}`}>Depan</button>
                        <button onClick={() => setActiveAngle('samping')} className={`text-[10px] font-black uppercase tracking-widest ${activeAngle === 'samping' ? 'text-accent' : 'text-gray-400'}`}>Samping</button>
                        <button onClick={() => setActiveAngle('belakang')} className={`text-[10px] font-black uppercase tracking-widest ${activeAngle === 'belakang' ? 'text-accent' : 'text-gray-400'}`}>Belakang</button>
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="mb-8">
